...
class RedisConnector extends EventEmitter {
  constructor(options) {
    ...
    this._redisSubscriber.on('messageBuffer', this._listeners.onMessage)
    this._redisSubscriber.on('pmessageBuffer', this._listeners.onPatternMessage)
  ...
  _onPatternMessage(pattern, channel, rawMessage) {
    ...
    debug('received raw message %o on channel %s for pattern %s',
      rawMessage.toString(), channel, pattern)
    ...
    if (rawMessage !== undefined) {
      ...
      if (channel.match('__keyspace')) {
        ...
        this.emit('key', key, message.toString())

        debug('key %s update with message %s', key, message.toString())
      } else {
        if (!channel.match('__stream')) {
          try {
            message = this.encoder.unpack(message)
    ...
    return new Promise((resolve, reject) => {
      if (!keys) {
        ...
        this._redis.mget(keys).then(values => {
          if (values) {
            values = values.map(e => JSON.parse(e))
          }
          resolve(values)
        }).catch(err => reject(err))
      }
  ...
  set(key, value) {
    return new Promise((resolve, reject) => {
      if (key) {
        this._redis.set(key, JSON.stringify(value)).then(() => {
          resolve(value)
        }).catch(err => reject(err))
      } else {
