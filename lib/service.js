...
class Service extends EventEmitter {
  ...
  _registerOneMethod(method) {
    return new Promise((resolve, reject) => {
      ...
      if (!!lookup) {
        httpAPICallType = httpAPICallType || lookup[1].toLowerCase()
                ...
                if (this._options.accesscontrol) {
                  try {
                    if (request.headers && request.headers.privileged) {
                      ...
                      grants = await this._ac.rolePermission('__privileged__',
                        this._name, this._permissionActions[method])

                      grantsAny = await this._ac.rolePermission('__privileged__',
                    ...
                    return { err: 'access not granted' }
                  }
                  
                  if (!grants.granted) return { err: 'access not granted' }
                }
      ...
      try {
        ...
        if (this._ac) {
          try {
            ...
            console.log('//////////////////failed to destroy access control', err)
          }
        }
