...
class Service extends EventEmitter {
  constructor(options) {
    ...
    this._connector.on('key', this._listerners.onKey)
  ...
  _onMessage(channel, message) {
    ...
    if (message.meta.req) {
      if (this._subscribedMethods[channel] &&
        typeof this._subscribedMethods[channel](message) === 'function') {
    ...
    try {
      ...
      if (operation === 'set') {
        this._connector.get(key).then(value => {
          this._api[serviceShortName] = this._connector.encoder.unpack(value)
          this.emit('service:up', serviceShortName, this._api[serviceShortName])
        })
  ...
  _registerMethods() {
    return new Promise(async (resolve, reject) => {
      ...
      this._connector.set(this._namespace + ':' + this._name, serviceDiscoveryInfo)
        .then(() => {
