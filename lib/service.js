        ...
        if (this._httpMethods[httpAPICallType] &&
          ...
          if (typeof this._rest[httpAPICallType] === 'function') {
            let cb = async (request, content) => {
              ...
              try {
                let result = await this[method](args, userId)
                return result
              } catch (err) {
                return err
              }
  ...
  _registerMethods() {
    return new Promise(async (resolve, reject) => {
      ...
      if (this._connector) {
        ...
        debug('connector not available: switch to HTTP discovery')
        ...
        if (this._options.discoveryServers) {
          ...
          for (let server of this._options.discoveryServers) {
            try {
              await got('/register', {
                baseUrl: (server.protocol || 'http') + '://' + server.host +
                  ':' + server.port,
                method: 'post',
                json: true,
              ...
              debug('failed to register in HTTP mode on discovery server %o',
                server)
            }
          }
