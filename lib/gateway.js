...
class Gateway extends Service {
  constructor(options) {
    ...
    // local methods bindings for function composition.
    // local methods are methods that are native to this gateways when
    // considered as a service (= as a provider)
    this._methodsBindings = {}
    ...
    return new Promise((resolve, reject) => {
      super._init().then(() => {
        for (let method in this._methods) {
          this[method] = () => {
            return new Promise((resolve, reject) => {
              if (this._methodsBindings[method]) {
                this._methodsBindings[method]().then(result => {
                  this[method](result).then(fResult => {
                    resolve(fResult)
  ...
  _addMethod(serviceName, methodName) {
    ...
    if (this._services[serviceName].pubsubRPCEnabled) {
      remoteCall = (...args) => {
      ...
      remoteCall = (...args) => {
        return new Promise((resolve, reject) => {
          ...
          got('/' + methodName, {
            ...
            method: httpCallType,
            json: true,
            body: { rq: this.encoder.pack({ args: args }).toString('base64') },
            ...
            if (response.body && response.body.err) {
              ...
              if (response.body) {
                res = this.encoder.unpack(Buffer.from(response.body.result, 'base64')).result
              }
  ...
  /* bind a local method to a remote one (function composition) */
  bindMethods(method, remoteService, remoteMethod, grants) {
    return new Promise((resolve, reject) => {
      // WARNING: applied ggrants here corresponds to the method caller's ones !
      this._methodsBindings[method] = ggrants => {
        return new Promise((rresolve, rreject) => {
          this._waitForServiceAPI(remoteService).then(service => {
            service.callEventuallyBoundMethod(remoteMethod, ggrants).then(result => {
              this[method](result).then(rresult => {
                rresolve(rresult)
              }).catch(err => rreject(err))
            }).catch(err => rreject(err))
          }).catch(err => rreject(err))
        })
      }
  ...
  /* unbind a local method from a remote one */
  unbindMethods(method, remoteService, remoteMethod) {
    return new Promise((resolve, reject) => {
      this._methodsBindings[method] = undefined
      resolve()
    })
  }
  ...
  /*
    ...
    method to a third service method call and so on.
    overwrites service method
  */
  callEventuallyBoundMethod(method, grants) {
    if (this[method]) {
      if (this._methodsBindings[method]) {
        return this._methodsBindings[method](grants)
      } else {
        return this[method]()
      }
    } else {
